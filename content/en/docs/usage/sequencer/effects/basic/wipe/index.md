...
* **Color Handling** This controls how the colors are applied on the effect. The simplest is a single color which will create a simple Wipe with only that color on it. The other three options allow for a [Color Gradient]({{< ref color-gradients >}} "Color Gradients") to be applied in different ways on the effect. See [Inline Gradient Editor]({{< ref inline-gradient-editor >}} "Inline Gradient Editor"). Only available when the **Type** **Movement** is set to **Count** or **Pulse Length**.
