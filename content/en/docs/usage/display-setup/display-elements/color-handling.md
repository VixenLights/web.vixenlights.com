...
1. Traditional analog incandescent, or LED string lights. These have been around for a long time and are typically a string of single color lights.
2. Dumb Pixels. These are LED pixels, but the whole string color can be controlled and will all be the same color at the same time. These lights are not individually addressable.
...
2. Multiple colors that do not mix. This is for what is known as super strings of traditional lights. A single prop may have multiple strings of different color lights covering the same area. You would configure this to have the set of colors that match what your strings are.
