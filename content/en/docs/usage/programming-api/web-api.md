...
Vixen implements a Restful Web API. The API allows you to get element info, search for elements, start/pause/stop a sequence, stop a running sequence and get status updates of running sequences.
...
_duration:_ The time in seconds to stay on. 0 is indefinite.
...
```js
self.initSystemStatusHub = function () {
