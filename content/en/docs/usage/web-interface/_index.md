...
To utilize this feature, validate under **Tools** -> **Web Server** -> **Configure** that the server is enabled. There you will find the address and the port the server runs on. Generally the default port is satisfactory, but if you have other applications utilizing the default port, you may need to change it. The server can be disabled if not needed.
...
![Web Main Screen](web-main-page.png)
...
![Web Elements Tree](web-main-page-ext.png)

Clicking on the name will expand the controls for the element. Here the element can be turned on for an interval, turned on for an extended period and turned off if previously turned on via the interface.

![Web Element Expanded](web-element-expand.png)

The color can be chosen to turn the element on for. If it is discrete, the configured colors will be configured to select.

![Web Color](web-element-color.png)

If it is a full color element, the color chooser will provide for the standard colors plus any custom color.

![Web RGB Color](web-element-ext-color.png)
...
![Web Intensity Control](web-intensity-control.png)
...
![Web Sequence Control](web-sequence-control.png)
